...
.evo-card { background:#f0fdf4; border-left:3px solid #22c55e; padding:10px 13px; border-radius:7px; font-size:.82rem; color:#166534; margin-top:6px; line-height:1.55; }
...
def _lazy_feedback():
    ...
    return record_rating, get_stats, get_recommendation, check_abuse, get_qa_pairs_count, export_qa_pairs_to_docs


def _invalidate_cache():
    for key in ("_cached_stats", "_cached_vs", "_cached_qa_count"):
...
def _cached_stats():
    _, gs, _, _, _, _ = _lazy_feedback()
    return gs()


@st.cache_data(ttl=30)
def _cached_qa_count():
    _, _, _, _, gqc, _ = _lazy_feedback()
    return gqc()
...
def render_feedback(idx, question, answer):
    ...
    with c1:
        if st.button("👍 有帮助", key=f"p_{idx}", help="标记此回答有帮助"):
            _, fb_check, _, _, _, _ = _lazy_feedback()
            allowed, reason = fb_check(question, st.session_state.session_ratings_count)
            if not allowed:
                st.session_state._fb_err = reason; st.rerun()
            else:
                rr, _, _, _, _, _ = _lazy_feedback()
...
with st.sidebar:
    st.markdown("### ⚙️ 参数设置")
    k = st.slider("**检索数量 k**", 1, 10, 3, help="越大回答越详细")
    ...
    with st.expander("🔬 进阶参数", expanded=False):
        similarity_threshold = st.slider("A3·拒答阈值", 0.0, 1.0, 0.0, 0.05, help="低于此值返回无法回答(0关闭)")
    ...
    if stats["total"] > 0:
        with st.expander("🧬 自进化分析", expanded=False):
            _, _, fb_rec, _, _, _ = _lazy_feedback()
            st.markdown(fb_rec())
            qac = _cached_qa_count()
            st.info(f"高分问答对: **{qac}** 条")
            if st.button("📤 导出问答对", use_container_width=True):
                _, _, _, _, _, ep_fn = _lazy_feedback()
                ep = ep_fn()
                if ep > 0:
                    st.success(f"已导出 {ep} 条 → 运行 `python ingest.py --incremental` 补充知识库")
                else:
                    st.warning("暂无高分问答对")
    else:
...
st.caption("基于 RAG 技术的本地知识库问答系统 · 支持 Python / LangChain 文档智能检索")
...
if not st.session_state.messages:
    ...
    <div class="welcome-box">
        ...
        📚 文档相关问题自动检索知识库回答 · 💬 日常对话直接与 AI 聊天 · 支持多轮对话与来源追溯</p>
    </div>
    """, unsafe_allow_html=True)
